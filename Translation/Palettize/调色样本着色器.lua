--
-- 作者:projectitis
-- 来源于:https://github.com/projectitis/aseprite-script-palettize
--
-- 将调色板应用于图像。
--
-- 如何安装捏?
--   1) 打开 Aseprite
--   2) 然后依次打开 文件 > 脚本 > 打开脚本文件夹
--   3) 复制此脚本到辣个文件夹
--   4) 脚本文件找不到? 打开 Aseprite, 再次点击 文件 > 脚本 > 刷新脚本文件夹(或者按 F5 刷新)
--   5) 运行这个脚本: 文件 > 脚本 > 调色样本着色器
--
-- 怎么打开鸭?
--   1) 文件 > 脚本 > 调色样本着色器
--
...
if sprite == nil then
    return app.alert("错误:没有可以用的活动文档，嗯，你是直接在主页上打开的吗？")
end
if sprite.colorMode == ColorMode.Tilemap then
    return app.alert("错误:瓦片图像的颜色模式不支持")
end
...
if image == nil then
    return app.alert("错误:当前单元格为空画布，无法操作")
end
...
-- (重新)绘制对话框
showDialog = function()
    local dlg = Dialog{ title = "调色样本着色器 - Palettize" }
    dlg:file{ id="src", label="源样本",
        label="样本文件",
        title="浏览文件",
        open=true,
        hexpand=false,
